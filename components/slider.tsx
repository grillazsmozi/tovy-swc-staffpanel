...
const Slider: FC<Props> = ({ children, activeSlide }: Props) => {
  ...
  useEffect(() => {
    if (elementsRef.current[activeSlide]) {
      ...
      >
        {Children.map(children, (child, index) => {
          return (
            <div
              key={index}
              className="w-full my-auto bg-white dark:bg-gray-800 dark:bg-opacity-50 dark:backdrop-blur-lg shrink-0 p-6 rounded-3xl"
              id={index.toString()}
